...
const columns = [
    { key: 'order', label: '#', noWrap: true },
    { key: 'icon', label: 'Icon', render: (val) => <span style={{ fontSize: '1.3rem' }}>{val}</span> },
    { key: 'title', label: 'Service', primary: true },
...
export default function ServicesListPage() {
    ...
    return (
        <div>
            <PageHeader title="Services" subtitle={`${data.length} services`} actionLabel="Add Service" actionTo="/services/new" />
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden' }}>
                <DataTable columns={columns} data={data} loading={loading} onEdit={r => navigate(`/services/edit/${r._id}`)} onDelete={setDeleteId} />
            </div>
            <ConfirmDialog open={!!deleteId} onConfirm={confirmDelete} onCancel={() => setDeleteId(null)} message="This service will be permanently deleted." />
        </div>
    );
}